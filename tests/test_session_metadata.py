from personal_knowledge_agent.agent_context.conversation_sessions import ConversationSessionMetadataRepository


def test_session_metadata_load_or_create_writes_default_metadata(tmp_path):
    store = ConversationSessionMetadataRepository(tmp_path, model="deepseek-test")
...
def test_session_metadata_updates_counts_and_restore_mode(tmp_path):
    store = ConversationSessionMetadataRepository(tmp_path)
...
def test_session_metadata_lists_sessions_by_updated_at(tmp_path):
    older = ConversationSessionMetadataRepository(tmp_path, session_id="older")
    newer = ConversationSessionMetadataRepository(tmp_path, session_id="newer")

    older.rename_session("旧会话")
    newer.rename_session("新会话")

    sessions = ConversationSessionMetadataRepository(tmp_path).list_sessions()
...
def test_session_metadata_renames_session_and_preserves_user_title(tmp_path):
    store = ConversationSessionMetadataRepository(tmp_path, session_id="named")
...
def test_session_metadata_generates_auto_title_from_user_message(tmp_path):
    store = ConversationSessionMetadataRepository(tmp_path, session_id="auto")
...
def test_session_metadata_auto_title_uses_first_user_message(tmp_path):
    store = ConversationSessionMetadataRepository(tmp_path, session_id="auto_once")
