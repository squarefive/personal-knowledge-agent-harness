...
from personal_knowledge_agent.agent_observability import AgentEventJsonlLogger


def test_async_jsonl_logger_writes_full_user_input(tmp_path):
    log_path = tmp_path / "agent.log"
    logger = AgentEventJsonlLogger(path=log_path)
    user_input = "原始输入" * 200
...
def test_async_jsonl_logger_drops_when_queue_is_full(tmp_path, capsys):
    log_path = tmp_path / "agent.log"
    logger = AgentEventJsonlLogger(path=log_path, max_queue_size=1)
