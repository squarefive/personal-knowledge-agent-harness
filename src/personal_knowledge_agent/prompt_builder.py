    ...
    sections = [
        "\n".join(
            [
                ...
                "当用户要记录知识时，保留原始 question 和 answer，生成 summary、keywords 和 category，然后调用 save_qa_card。",
                "keywords 是检索词；category 是这张卡片唯一的语义主归属分类。",
                "category 必填，必须是具体稳定的短名词短语；不得使用“其他”“未分类”“杂项”“默认分类”“未知”“待分类”等兜底分类。",
                ...
                "搜索时 category 是可选硬过滤条件；只有用户明确限定分类时，才给 search_qa_cards、hybrid_search_qa_cards 或 list_recent_cards 传 category。",
