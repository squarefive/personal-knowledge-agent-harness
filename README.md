...
created_at: 创建时间
updated_at: 更新时间
is_vectorized: 是否已写入语义索引
```

当前不包含 tags。后续如有需要，再增加标签、置信度、版本、关联卡片等字段。
...
当前已完成最小 Q&A 闭环、可信来源闭环、Q&A 更新删除、Hybrid 检索、category 分类和本地 Web Chat + Cards 基础能力。项目仍不急于扩展 Wiki、后台任务、多 Agent 或复杂知识管理能力。
...
| `v0.5` | 分类 | `qa_cards` 增加必填 category；保存时由模型生成，用户可通过 update 工具手动修改；本阶段不新增 tags，keywords 继续承担检索词职责 | 已完成 |
...
| 模块 | 状态 | 当前说明 |
|---|---|---|
| Q&A 知识管理 | 部分完成 | 已支持保存 Q&A 卡片、关键词检索、hybrid 检索、读取卡片、最近卡片列表、更新、删除和 category；尚未支持标题、标签、导入和导出。 |
...
| 来源追踪 | 部分完成 | Q&A 工具返回 `card_id`、原始问题、`source_type` 和创建时间；程序会从本轮真实工具结果重写来源区块并清理无证据来源声明；尚未支持多来源类型。 |
...
| Web Chat + Cards | 部分完成 | 已实现本地 HTML 聊天入口、`POST /api/chat/stream`、最近卡片、卡片搜索和卡片详情；不包含编辑、删除、合并或复杂知识图谱。 |

整体判断：当前项目已完成第一版最小 Q&A 闭环和部分 Harness 基础，距离最终版完整个人知识系统还有较大差距。优先缺口是 Q&A 导入导出、统一检索、标签体系、Wiki 管理和更完整的权限审计。
...
- `docs/agents/`: 具体 Agent 的稳定开发边界文档目录，不保存任务计划。
